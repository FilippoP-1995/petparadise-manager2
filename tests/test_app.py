...
class PetParadiseTests(unittest.TestCase):
    ...
    def test_invoice_total_recomputes_on_preventivo_changes_until_manually_overridden(self):
        # "Totale fattura" segue sempre "Totale W" finche' non esiste una
        # fattura vera (numero fattura o FARE FATTURA) E l'utente non lo ha
        # modificato di persona (invoice_total_manual): a quel punto il
        # valore scritto a mano resta, esattamente come richiesto
        # dall'utente ("deve essere sempre modificabile").
        stamp = "2026-07-15T10:00:00"
        with app.db() as conn:
            admin = conn.execute("SELECT * FROM users WHERE username='admin'").fetchone()
            pid = conn.execute(
        ...
        # A deliberate manual edit (invoice_total_manual="Si", set client-side
        # only when the user types directly into the field) must stick — a
        # further preventivo change must NOT resync it back to the total.
    ...
    def test_reported_bug_invoice_total_realigns_via_salva_pagamento_w(self):
        ...
        # vera emessa e nessun override manuale attivo. Il pulsante "Salva
        # pagamento W" (submit dell'intero practiceForm) deve riallineare
        # "Totale fattura" al totale reale.
        stamp = "2026-07-15T10:00:00"
        with app.db() as conn:
            admin = conn.execute("SELECT * FROM users WHERE username='admin'").fetchone()
            pid = conn.execute(
    ...
    def test_invoice_total_manual_override_sticks_via_salva_pagamento_w(self):
        # Complemento del test precedente: se invoice_total_manual="Si" e'
        # esplicitamente presente (l'utente ha scritto a mano il totale
        # fattura), "Salva pagamento W" NON deve piu' riallinearlo al totale
        # reale — richiesta esplicita dell'utente, il campo deve restare
        # modificabile e il valore scritto a mano deve restare.
        stamp = "2026-07-15T10:00:00"
        with app.db() as conn:
            admin = conn.execute("SELECT * FROM users WHERE username='admin'").fetchone()
            pid = conn.execute(
                """INSERT INTO practices(practice_number,request_origin,destination_branch,status,created_at,updated_at,
                   ...
                   owner_city,owner_province,owner_zip,total_service,invoice_total,invoice_total_manual,payment_status)
                   VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                ("CR-MANUALOK", "Privato", "Livorno", "Ritirato", stamp, stamp, admin["id"], "Buddy",
                 "Anna", "Bianchi", "3339990000", "X", "Via Roma", "Livorno", "LI", "57100",
                 "310.00", "274.00", "", "Da saldare"),
            ).lastrowid
        self.handler.redirect = lambda path: None
        ...
        with app.db() as conn:
            row = conn.execute("SELECT total_service,invoice_total FROM practices WHERE id=?", (pid,)).fetchone()
        self.assertEqual((row["total_service"], row["invoice_total"]), ("310.00", "274.00"))
    ...
    def test_calendar_nuovo_prefills_riconsegna_in_sede_from_linked_practice_id(self):
        # richiesta esplicita dell'utente: il bottone "Fissa riconsegna" deve
        # portare dritto al wizard normale di creazione evento (non un popup
        # ristretto che chiede solo data/ora), con tipo pre-impostato su
        # "Riconsegna in sede" e i dati della pratica (anche il pagamento)
        # gia' ripresi ma tutto restando liberamente modificabile.
        with app.db() as conn:
            admin = conn.execute("SELECT * FROM users WHERE username='admin'").fetchone(); stamp = app.now()
            pid = conn.execute("""INSERT INTO practices(practice_number,request_origin,destination_branch,status,created_at,updated_at,created_by,
                                animal_name,owner_first_name,owner_last_name,owner_phone,total_service,total_service_manual,payment_status)
                                VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                                ("CR-PREFILL","Privato","Livorno","Consegnato",stamp,stamp,admin["id"],"Sam","Laura","Barone","3339990000",
                                 "240.00","Si","Pagato")).lastrowid
        ...
        page=rendered[-1]
        self.assertIn('value="Riconsegna in sede" checked', page)
        self.assertIn(f'name="linked_practice_id" value="{pid}"', page)
        self.assertIn('value="Sam"', page)
        self.assertIn('value="Laura"', page)
        self.assertIn('value="Barone"', page)
        self.assertIn('value="3339990000"', page)
        self.assertIn('value="240.00"', page)

    def test_calendar_nuovo_linked_practice_id_respects_explicit_event_type_override(self):
        # l'utente deve poter scegliere liberamente un altro tipo (es.
        # "Riconsegna" a domicilio) invece del default "Riconsegna in sede".
        with app.db() as conn:
            admin = conn.execute("SELECT * FROM users WHERE username='admin'").fetchone(); stamp = app.now()
            pid = conn.execute("""INSERT INTO practices(practice_number,request_origin,destination_branch,status,created_at,updated_at,created_by,animal_name)
                                VALUES(?,?,?,?,?,?,?,?)""",("CR-OVERRIDE","Privato","Livorno","Consegnato",stamp,stamp,admin["id"],"Sam")).lastrowid
        rendered=[];self.handler.send_html=lambda html,*a:rendered.append(html)
        self.handler.path=f"/calendario/nuovo?linked_practice_id={pid}&event_type=Riconsegna"
        self.handler.calendar_event_form(admin)
        page=rendered[-1]
        self.assertIn('value="Riconsegna" checked', page)
        self.assertNotIn('value="Riconsegna in sede" checked', page)

    def test_calendar_nuovo_ignores_unknown_linked_practice_id(self):
        with app.db() as conn:
            admin = conn.execute("SELECT * FROM users WHERE username='admin'").fetchone()
        rendered=[];self.handler.send_html=lambda html,*a:rendered.append(html)
        self.handler.path="/calendario/nuovo?linked_practice_id=999999"
        self.handler.calendar_event_form(admin)
        self.assertNotIn('value="Riconsegna in sede" checked', rendered[-1])

    def test_schedule_delivery_no_longer_uses_removed_popup(self):
        # il vecchio popup "Fissa riconsegna" (solo data/ora, tipo fisso su
        # "Riconsegna") e' stato rimosso: il riepilogo pratica non deve piu'
        # contenerlo, e savePracticeState naviga direttamente al wizard
        # normale invece di aprirlo.
        with app.db() as conn:
            admin = conn.execute("SELECT * FROM users WHERE username='admin'").fetchone(); stamp = app.now()
            pid = conn.execute("""INSERT INTO practices(practice_number,request_origin,destination_branch,status,created_at,updated_at,created_by,animal_name)
        ...
        self.handler.practice(admin,pid)
        page=rendered[-1]
        self.assertNotIn('id="scheduleDeliveryBackdrop"',page)
        js=app.APP_JS
        self.assertNotIn("function ppmOpenScheduleDeliveryModal", js)
        self.assertNotIn("function ppmCloseScheduleDeliveryModal", js)
        ...
        self.assertIn("window.location.href='/calendario/nuovo?linked_practice_id='+practiceMatch[1]",save_state_fn)
    ...
    def test_cremation_day_view_shows_vedi_riconsegna_when_delivery_event_already_exists(self):
        with app.db() as conn:
            admin = conn.execute("SELECT * FROM users WHERE username='admin'").fetchone(); stamp = app.now()
            cycle_id = conn.execute(
                "INSERT INTO cremation_cycles(cycle_date,status,planned_start,planned_end,actual_end,created_at,updated_at) VALUES(?,?,?,?,?,?,?)",
            ...
            pid = conn.execute(
                """INSERT INTO practices(practice_number,request_origin,destination_branch,status,service_type,
                   pickup_date,created_at,updated_at,created_by,animal_name,cremation_cycle_id) VALUES(?,?,?,?,?,?,?,?,?,?,?)""",
                ("CR-VEDI1", "Privato", "Livorno", "Da consegnare", "Cremazione singola", "2026-07-20", stamp, stamp,
                 admin["id"], "Sam", cycle_id),
            ).lastrowid
            event_id = conn.execute(
                """INSERT INTO calendar_events(event_type,title,animal_name,linked_practice_id,operator_name,start_at,end_at,event_status,created_by,created_at,updated_at)
                   VALUES(?,?,?,?,?,?,?,?,?,?,?)""",
                ("Riconsegna in sede","RICONSEGNA SAM","Sam",pid,"Serena","2026-07-21T10:00:00","2026-07-21T10:30:00","In programma",admin["id"],stamp,stamp),
            ).lastrowid
        rendered = []
        self.handler.path = "/programma-cremazioni?data=2026-07-20"
        self.handler.send_html = lambda content, *args: rendered.append(content)
    ...
    def test_invoice_total_formats_two_decimals_without_euro_sign_in_value(self):
        ...
        self.assertIn("invoiceTotal.value=serviceTotal?ppmFormatInvoiceTotal(serviceTotal):'';", js)
        self.assertIn("invoiceTotal.addEventListener('blur'", js)

    def test_invoice_total_field_is_always_editable_and_auto_synced_until_manually_overridden(self):
        # Bug segnalato dall'utente: "Totale fattura" non seguiva "Totale W"
        # e non era mai modificabile a mano. Ora il campo NON e' mai
        # readOnly, e segue Totale W in automatico finche' l'utente non lo
        # tocca di persona (invoice_total_manual, stesso meccanismo gia'
        # usato per total_service_manual) o non esiste gia' una fattura
        # vera emessa.
        js = app.APP_JS
        self.assertIn("function ppmInvoiceAlreadyIssued(){", js)
        self.assertNotIn("invoiceTotal.readOnly", js)
        self.assertIn("const invoiceTotalManual=document.querySelector('input[name=\"invoice_total_manual\"]');", js)
        self.assertIn("invoiceTotal.addEventListener('input',()=>{ if(invoiceTotalManual) invoiceTotalManual.value='Si'; });", js)

    def test_invoice_total_autofollows_totale_w_not_acconto_plus_saldo(self):
        # Bug segnalato dall'utente: "Totale fattura" restava fermo quando
        # si modificava "Totale W" e cambiava solo toccando "Rimanenza"
        # (Saldo/Rimanenza W), perche' la formula sommava Acconto W + Saldo
        # W invece di seguire il totale reale. Ora segue direttamente Totale
        # W (serviceTotal), sia nel ricalcolo live sia al caricamento pagina.
        js = app.APP_JS
        self.assertIn("invoiceTotal.value=serviceTotal?ppmFormatInvoiceTotal(serviceTotal):'';", js)
        self.assertIn("invoiceTotal.value=(totalService&&ppmNumber(totalService.value))?ppmFormatInvoiceTotal(totalService.value):'';", js)
        self.assertNotIn("invoiceTotal.value=ppmFormatInvoiceTotal(accontoW+saldoW);", js)
    ...
    def test_swipe_back_from_the_revealed_actions_row_also_closes_it(self):
        # richiesta esplicita dell'utente: deve poter chiudere le azioni
        # rivelate anche trascinando in direzione opposta (verso destra), non
        # solo toccando fuori dalla card. Il gesto di trascinamento deve
        # partire dall'intera slide (azioni comprese), non solo dal lembo di
        # card frontale ancora visibile.
        js = app.APP_JS
        fn = js[js.index("function setupReminderSwipe(slide){"):]
        fn = fn[:fn.index("\nfunction ", 10)]
        self.assertIn("slide.addEventListener('pointerdown',function(e){", fn)
        self.assertIn("slide.addEventListener('pointermove',function(e){", fn)
        self.assertIn("slide.addEventListener('pointerup',finish);", fn)
        self.assertIn("downTarget=e.target;", fn)
        self.assertIn("wasOpen&&target&&front.contains(target)", fn)
        self.assertIn("touch-action:pan-y", app.CSS[app.CSS.index(".reminders-swipe-actions{"):app.CSS.index(".reminders-swipe-actions{")+200])
